...
public class FraudCaseAssigneeChangeRequestDeserializer
        extends JsonDeserializer<FraudCaseAssigneeChangeRequest> {

    private static final Set<String> ALLOWED_FIELDS = Set.of(
            "assigneeRef", "reasonCode", "expectedVersion"
    );

    @Override
    public FraudCaseAssigneeChangeRequest deserialize(
            JsonParser parser,
        ...
        while (parser.nextToken() != JsonToken.END_OBJECT) {
            ...
            switch (field) {
                ...
                default -> throw new IllegalStateException(
                        "Allowed assignee request field is not mapped"
                );
            }
        }
        if (parser.nextToken() != null) {
            throw mapping(
                    ...
                    "$",
                    "INVALID_JSON_TOKEN",
                    "Case assignee request contains trailing JSON content"
            );
        }
        return new FraudCaseAssigneeChangeRequest(
                assigneeRef,
                assigneeRefPresent,
                reasonCode,
                expectedVersion
    ...
    @Override
    public FraudCaseAssigneeChangeRequest getNullValue(
            DeserializationContext context
    ) throws JsonMappingException {
        throw mapping(context.getParser(), "$", "INVALID_JSON_ROOT",
                "Case assignee request JSON root must be an object");
    }
    ...
    private String requireField(JsonParser parser, Set<String> seen)
        ...
        if (parser.currentToken() != JsonToken.FIELD_NAME) {
            throw mapping(parser, "$", "INVALID_JSON_TOKEN",
                    "Case assignee request JSON contains an invalid token");
        }
        String field = parser.currentName();
        if (!ALLOWED_FIELDS.contains(field)) {
            throw mapping(parser, field, "UNKNOWN_JSON_FIELD",
                    "Unknown case assignee request field");
        }
        if (!seen.add(field)) {
            throw mapping(parser, field, "DUPLICATE_JSON_FIELD",
                    "Duplicate case assignee request field");
        }
        return field;
    }
    ...
    private String readString(
        ...
        if (token != JsonToken.VALUE_STRING) {
            throw mapping(parser, field, "INVALID_JSON_TOKEN",
                    "Case assignee request field has an invalid JSON type");
        }
