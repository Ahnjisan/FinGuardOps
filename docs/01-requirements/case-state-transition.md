...
Issue #154에서 사건 영속 기반에 필요한 Java Enum·Entity와 Flyway V6의
`fraud_case`·`case_transaction` 테이블, FK·Unique·Check·Index는 구현되었다.
구체적인 물리 계약은
[`fraud-case-schema.md`](../04-database/fraud-case-schema.md)를 따른다.

다음 항목은 여전히 확정하거나 구현하지 않는다.

- 사건 조사 상태 전이 Service와 공개 API
...
초기 계약에서 `OPEN` 사건은 담당자가 없을 수 있지만 `IN_REVIEW` 사건에는 담당자가 반드시 있어야 한다. `OPEN`에서 `IN_REVIEW`로 전이할 때 상태 변경 요청의 `assigneeRef`로 담당자를 함께 지정하고, `ADDITIONAL_INFORMATION_REQUIRED`에서 `IN_REVIEW`로 복귀할 때는 기존 담당자가 있어야 한다. 이미 `IN_REVIEW`인 사건의 재배정은 별도 담당자 변경 API를 사용한다. 자동 배정 도입 여부는 `TBD`이다.
...
사건 감사를 중복 기록하지 않는다. 조사 상태 전이 감사, 조회 API, 보존 기간과 접근
범위는 확정하지 않는다.
...
append-only AuditLog 물리 기반을 구현했다. 위험 대응 최종화의 신규 사건·첫 연결
감사도 구현되었다. 조사 상태 전이 Service, 공개 API DTO·상태 코드와 그 변경의
AuditLog 통합은 후속 승인 범위이다.
