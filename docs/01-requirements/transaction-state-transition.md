...
이 문서는 요구사항 단계의 상태와 전이를 정의한다. 거래 접수의 DB 컬럼·제약조건·멱등성 키와 낙관적 잠금은 후속 물리 계약인 [`../04-database/transaction-intake-schema.md`](../04-database/transaction-intake-schema.md)에서 확정한다.
