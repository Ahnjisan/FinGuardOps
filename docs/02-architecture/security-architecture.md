...
화면이 아직 구현되지 않았다. Issue #259가 사건 상세 내부 read-only notes section을 추가했고,
Issue #261은 같은 section에 capability·사건 상태로 제한한 inline create composer를 추가했다. 별도 notes
route와 update/delete UI는 계속 구현하지 않는다.
...
반사하지 않는다. Issue #261의 POST client는 exact 201과 requested caseId·submitted content·safe
`expectedVersion + 1` 결합을 추가로 확인하고 authorRef를 Frontend session subject와 결합하지 않는다.
...
않는다. Issue #261에서 공개 반환에는 기존 `state`·`retry`에 explicit `refresh`·`refreshState`가 추가되며,
...
session identity·caseId·expectedVersion·submission generation이 모두 current인 callback만 terminal state와
reconciliation을 게시한다. released/already-settled flight는 projection/classification factory를 실행하지
