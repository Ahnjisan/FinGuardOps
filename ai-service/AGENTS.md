...
## Rule·ML과 생성형 AI 책임 구분

### Rule·ML 책임

- Feature를 계산한다.
- Rule을 실행하고 ML 추론을 수행한다.
- 위험 점수를 계산한다.
- 탐지 근거와 Reason Code를 생성한다.
- 모델 평가 지표를 기록한다.

### 생성형 AI 책임

- HIGH·CRITICAL 사건을 중심으로 사건 리포트를 생성한다.
- 탐지 근거와 사용자 행동 타임라인을 요약하고 담당자 확인 항목을 제시한다.
- 사건 복잡도에 따라 모델을 라우팅하고 입력 데이터를 축약한다.
- 출력 토큰과 재생성 횟수를 제한한다.
...
- 생성형 AI는 위험 점수 계산, 최종 이상거래 판정, 거래 승인·차단, 고객 제재와 사건 상태 자동 확정을 수행하지 않는다.
- 생성형 AI는 Rule 조건이나 가중치 또는 API 계약을 임의로 변경하지 않는다.
- 리포트 캐시는 정확 일치 방식만 사용하며 캐시 키는 `caseId + detectionResultVersion + promptVersion + modelVersion` 조합을 사용한다.
- Reason Code가 같다는 이유로 다른 사건의 리포트를 재사용하지 않는다.
- 시맨틱 유사도를 기준으로 사건 간 리포트를 재사용하지 않으며 범용 시맨틱 캐시를 도입하지 않는다.
- FDS 챗봇 또는 Investigation Copilot을 임의로 추가하지 않는다.
